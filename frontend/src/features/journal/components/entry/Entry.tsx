...
export function Entry({ entry, deleteEntry, selectEntry }: EntryProps): JSX.Element {
  const titleEmpty = entry.title === null || entry.title === ''
  const titleOrPlaceholder = titleEmpty ? JOURNAL_TITLE_PLACEHOLDER : entry.title
  return (
    <>
      <li className='entry-li' id={entry.id.toString()}>
        <Tooltip title={titleOrPlaceholder} placement="top">
          <p className={titleEmpty ? 'entry-title use-placeholder' : 'entry-title'}>
            {titleOrPlaceholder}
          </p>
        </Tooltip>
